...
use wayland_server::{
    ...
    Client, Dispatch, Display, GlobalDispatch,
};
...
impl DataDeviceState {
    pub fn new<D, L>(display: &mut Display<D>, logger: L) -> DataDeviceState
    where
        D: GlobalDispatch<wl_data_device_manager::WlDataDeviceManager>
            + Dispatch<wl_data_device_manager::WlDataDeviceManager, UserData = ()>
            + DataDeviceHandler,
        L: Into<Option<::slog::Logger>>,
    {
        let logger = crate::slog_or_fallback(logger).new(o!("smithay_module" => "data_device_mgr"));
        let global = display.create_global::<wl_data_device_manager::WlDataDeviceManager>(3, ());

        DataDeviceState { global, logger }
    }
...
pub trait DataDeviceHandler {
    ...
    fn event(&mut self, event: DataDeviceEvent);

    /// Selects an action for drag'n'drop negotiation.
    ///
    /// Implementations of this function may select the desired choice of action resulting from a drag'n'drop
    /// session.
    ///
    /// # Parameters
    ///
    /// * `available` specifies which set of actions are available.
    /// This corresponds to the intersection of actions supported by the source and targets.
    ///
    /// * `preferred` specifies the preferred action by the target.
    ///
    /// If no action should be selected, (and thus the drag'n'drop should abort on drop), return
    /// [`DndAction::empty()`].
    fn select_action(&mut self, available: DndAction, preferred: DndAction) -> DndAction {
